...
Copy_to_CLOVER_kext_dir="NO"                             # YES or NO
CLOVER_kext_dir="/Volumes/EFI/EFI/CLOVER/kexts/10.9"
...
# Default=1600 #For Pointer acceleration enhancement
...
########################  1Finger Gestures #########################
####################################################################
TrackpadTappingCorners="DEFAULT"      # YES or NO
CornerTapStartABS="DEFAULT"           # YES or NO
DisableBottomCornersTap="DEFAULT"     # YES or NO

FingLiftDragRelTimeOut="DEFAULT"      # TimeOut for releasing DragMode. Default=0.3 Secound
TapHoldDragPressure="DEFAULT"         # Pressure to activate DragMode. Default=0 Possible 0-100
TapHoldDragStartTimeOut="DEFAULT"     # Default=1.2 Secounds #Tap&Hold Drag lift TimeOut

EdgeSwipe="DEFAULT"                   # YES or NO
EdgeSwipeBottom="DEFAULT"             # NO=Disable / 1-32=Action
EdgeSwipeLeft="DEFAULT"               # NO=Disable / 1-32=Action
EdgeSwipeRight="DEFAULT"              # NO=Disable / 1-32=Action
EdgeSwipeTop="DEFAULT"                # NO=Disable / 1-32=Action
...
s2FingersPressAction="DEFAULT"        # NO=Disable / 1-32=Action

####################################################################
########################  3Finger Gestures #########################
####################################################################
s3FingerSwipes="DEFAULT"              # YES or NO
s3FingerSwipeDownAction="DEFAULT"     # NO=Disable / 1-32=Action
s3FingerSwipeLeftAction="DEFAULT"     # NO=Disable / 1-32=Action
s3FingerSwipeRightAction="DEFAULT"    # NO=Disable / 1-32=Action
s3FingerSwipeUpAction="DEFAULT"       # NO=Disable / 1-32=Action
s3FingersPressAction="DEFAULT"        # NO=Disable / 1-32=Action

####################################################################
########################  4Finger Gestures #########################
####################################################################
s4FingerSwipes="DEFAULT"              # YES or NO
s4FingerSwipeDownAction="DEFAULT"     # NO=Disable / 1-32=Action
s4FingerSwipeLeftAction="DEFAULT"     # NO=Disable / 1-32=Action
s4FingerSwipeRightAction="DEFAULT"    # NO=Disable / 1-32=Action
s4FingerSwipeUpAction="DEFAULT"       # NO=Disable / 1-32=Action
s4FingersPressAction="DEFAULT"        # NO=Disable / 1-32=Action
s4FingersTapAction="DEFAULT"          # NO=Disable / 1-32=Action

####################################################################
########################  5Finger Gestures #########################
####################################################################
s5FingersPinchAction="DEFAULT"        # NO=Disable / 1-32=Action
s5FingersPressAction="DEFAULT"        # NO=Disable / 1-32=Action
s5FingersTapAction="DEFAULT"          # NO=Disable / 1-32=Action
...
KeyBoardNumLockOn="DEFAULT"                         # YES or NO
...
Using_AsusBacklight="DEFAULT"                       # YES or NO
Enable_Asus_KBacklight="DEFAULT"                    # YES or NO
KBacklight_has_16_Levels="DEFAULT"                  # YES or NO
IdleAsusKBlightAutoOff="DEFAULT"                    # YES or NO
IdleAsusKBlightTimeOut="DEFAULT"                    # Timeout: Default=10 Secounds

Enable_Extended_Functions="DEFAULT"                 # YES or NO
Make_shift_and_caps_into_Fn_key="DEFAULT"           # YES or NO
Fn_keys_Mode="DEFAULT"                              # 1,2,3 DEFAULT=2
Fn_keys_Layout="DEFAULT"                            # Read Thread DEFAULT=NONE

#################### Fn Keys Mode 1 ####################
######## Possible Options: F1 - F12, ArrowRight, #######
#### ArrowDown, ArrowLeft, Pause, "NO"=Disable #########
m1_Misson_Control="DEFAULT"
m1_Launchpad="DEFAULT"
m1_KBBacklightDown="DEFAULT"
m1_KBBacklightUp="DEFAULT"
m1_BrightnessDown="DEFAULT"
m1_BrightnessUp="DEFAULT"
m1_MediaPrevious="DEFAULT"
m1_MediaPlayPause="DEFAULT"
m1_MediaNext="DEFAULT"
m1_VolumeMute="DEFAULT"
m1_VolumeDown="DEFAULT"
m1_VolumeUp="DEFAULT"
m1_SleepDisplay="DEFAULT"
m1_VideoMirror="DEFAULT"
m1_Touchpad="DEFAULT"
...
####################################################################
####################################################################
HasSensorALS="DEFAULT"                               # "YES" or "NO"
...
DimBacklightOnACDC="DEFAULT"                         # "YES" or "NO"
...
HasKeyboardBacklight="DEFAULT"                       # "YES" or "NO"
KeyboardHas16BLightLvls="DEFAULT"                    # "YES" or "NO"
KBackLightlvlAtBoot="DEFAULT"                       # 0-3 or 0-16 ...
IdleKBacklightAutoOff="DEFAULT"                      # "YES" or "NO"
IdleKBacklightAutoOffTimeout="DEFAULT"               # Timeout: Default=10 Secounds

HasMediaFnKeys="DEFAULT"                             # "YES" or "NO"
# Do you have special Media Keys? (Not Fn+F1-F13)

DisplayUsesNvidiaGPU="DEFAULT"                       # "YES" or "NO"

MediaKeysDoFastRewind="DEFAULT"                        # "YES" or "NO"
...
if [[ $Enable_Touchpad_Settings == "YES" ]]; then     # TOUCHPAD SETTINGS START #
	...
	if [[ $FingLiftDragRelTimeOut != "DEFAULT" && $FingLiftDragRelTimeOut != "YES" ]]; then
		if [[ $FingLiftDragRelTimeOut == "NO" ]]; then
		FingLiftDragRelTimeOut=0; fi
		sFingLiftDragRelTimeOut=$(echo "$FingLiftDragRelTimeOut*1000/1" | bc)
		if [[ "$sFingLiftDragRelTimeOut" -ge 0 && "$sFingLiftDragRelTimeOut" -le 10000 ]]; then
			perl -0777 -pe "s|<key>FingLiftDragRelTimeOut</key>\s*.*>|<key>FingLiftDragRelTimeOut</key>\n<integer>$sFingLiftDragRelTimeOut</integer>|" ~/$TOUCHPADPLIST > ~/$TOUCHPADPLIST2
			rm ~/$TOUCHPADPLIST; mv ~/$TOUCHPADPLIST2 ~/$TOUCHPADPLIST
			if [[ $FingLiftDragRelTimeOut == 0 ]]; then
			echo "--> FingLiftDragRelTimeOut - Disabled"; else
			echo "--> FingLiftDragRelTimeOut - set to $FingLiftDragRelTimeOut Secounds"; fi; fi; fi
	if [[ $TapHoldDragPressure != "DEFAULT" && $TapHoldDragPressure != "YES" ]]; then
		if [[ $TapHoldDragPressure == "NO" ]]; then
		TapHoldDragPressure=0; fi
		if [[ "$TapHoldDragPressure" -ge 0 && "$TapHoldDragPressure" -le 100 ]]; then
			perl -0777 -pe "s|<key>TapHoldDragPressure</key>\s*.*>|<key>TapHoldDragPressure</key>\n<integer>$TapHoldDragPressure</integer>|" ~/$TOUCHPADPLIST > ~/$TOUCHPADPLIST2
			rm ~/$TOUCHPADPLIST; mv ~/$TOUCHPADPLIST2 ~/$TOUCHPADPLIST
			echo "--> TapHoldDragPressure - set Presslevel to $TapHoldDragPressure"; fi
			# Need to modify here!!! ------###########
			fi
	if [[ $TapHoldDragStartTimeOut != "DEFAULT" && $TapHoldDragStartTimeOut != "YES" ]]; then
		if [[ $TapHoldDragStartTimeOut == "NO" ]]; then
		TapHoldDragStartTimeOut=0; fi
		sTapHoldDragStartTimeOut=$(echo "$TapHoldDragStartTimeOut*1000/1" | bc)
		if [[ "$sTapHoldDragStartTimeOut" -ge 0 && "$sTapHoldDragStartTimeOut" -le 10000 ]]; then
			perl -0777 -pe "s|<key>TapHoldDragStartTimeOut</key>\s*.*>|<key>TapHoldDragStartTimeOut</key>\n<integer>$sTapHoldDragStartTimeOut</integer>|" ~/$TOUCHPADPLIST > ~/$TOUCHPADPLIST2
			rm ~/$TOUCHPADPLIST; mv ~/$TOUCHPADPLIST2 ~/$TOUCHPADPLIST
			if [[ $TapHoldDragStartTimeOut == 0 ]]; then
			echo "--> TapHoldDragStartTimeOut - Disabled"; else
			echo "--> TapHoldDragStartTimeOut - set to $TapHoldDragStartTimeOut Secounds"; fi; fi; fi
